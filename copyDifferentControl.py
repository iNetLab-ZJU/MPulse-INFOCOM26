neighbor_graph_list = {
    11:{1:60,2:52,3:44,4:36,5:28,6:20,7:12,8:4,9:0,10:8},
...
def find_common_neighbors(graph):
    common_neighbors = {}
    ...
    for i in range(len(nodes)):
        for j in range(i+1, len(nodes)):
            ...
            neighbors1 = set(graph[node1].keys())
            neighbors2 = set(graph[node2].keys())
            common = neighbors1.intersection(neighbors2)
            common_neighbors[(node1, node2)] = common
    
    return common_neighbors


common_neighbors = find_common_neighbors(neighbor_graph_list)
# for pair, neighbors in common_neighbors.items():
#     print(f"Switches {pair} have common neighbors: {neighbors}")
...
for switch_id in switch_id_recognition:
    SWITCH_NUM=switch_id['SWITCH_NUM']
    if SWITCH_NUM!=11:
        continue
    print("11")
    switchNUM=[]
    ...
    STORE_TEXT=[]
    GET_NEIGHBOR_ALIVE_REG_ACTION=[]
    STORE_MONITERED_ALIVE_REG_ACTION_X1=[]
    STORE_MONITERED_ALIVE_REG_ACTION_X2=[]
    switchNUM.append("""control SwitchIngress_%d""""" % (SWITCH_NUM))
    for key in switch_id.keys() :
         if key.startswith('neighbor') :  
            mask.append("""bit<32> mask_%d;
                        """
                         "" % (switch_id[key]))
            GEN_REGISTER_TIME.append("""
                GEN_REGISTER_TIME(%d)"""
                         "" % (switch_id[key]))
            GEN_NEIGHBOR_STATE_STRING_REGISTER.append("""
                GEN_NEIGHBOR_STATE_STRING_REGISTER(%d)"""
                         "" % (switch_id[key])) 
            GET_NEIGHBOR_ALIVE_REG_ACTION.append("""
                GET_NEIGHBOR_ALIVE_REG_ACTION(%d,%d,%d)"""
                         "" % (num,switch_id[key],SWITCH_NUM))
            STORE_MONITERED_ALIVE_REG_ACTION_X1.append("""
                STORE_MONITERED_ALIVE_REG_ACTION_X(%d,%d,%d)"""
                         "" % (num,switch_id[key],SWITCH_NUM))
            num+=1
    STORE_TEXT.append("""
        //todo:
        // n : monitered switch id, signature:%d
        #ifndef STORE_MONITERED_ALIVE_REG_ACTION
        #define STORE_MONITERED_ALIVE_REG_ACTION(n)\\
                mask_##n## = (bit<32>)1 <<(%d-1);\\
                update_neighbor_alive_state_string_##n##_reg_action.execute(1);\\
                store_neighbor_time_##n##_reg_action.execute(%d);
        #endif
        ...
        #endif
    """
                      "" % (SWITCH_NUM,SWITCH_NUM,SWITCH_NUM,SWITCH_NUM,SWITCH_NUM))
    generated_text1 = []
    generated_text2 = []
    generated_text3 = []
    generated_text_control = []
    generated_text_control_2 = []
    for index, (key, value) in enumerate(switch_id.items()):
        if key.startswith('neighbor'):
            neighbor_num = key[-1]
            print(value)
            
            if neighbor_num == '0':
                generated_text1.append("""
                    if(reg_table_key_monitered==%d){
                        STORE_MONITERED_ALIVE_REG_ACTION_N(%d);
                    }"""
                "" % (switch_id[key],switch_id[key])) 
                generated_text_control.append("""
                    if(reg_table_key_monitered==%d){
                        STORE_MONITERED_ALIVE_REG_ACTION_N(%d);"""
                "" % (switch_id[key],switch_id[key]))
                neighborNum=0
                for neighbor in list(neighbor_graph_list[int(value)].keys()):
                        if neighbor==11:continue
                        generated_text_control.append("""
                    GET_NEIGHBOR_ALIVE_REG_ACTION(%d,%d,%d)"""
                        "" % (neighborNum,neighbor,SWITCH_NUM))
                        neighborNum+=1 
                generated_text_control.append("""
                    }"""
                ) 
                generated_text2.append("""
                    if(reg_table_key_monitered==%d){
                        STORE_MONITERED_ALIVE_REG_ACTION(%d);
                    }"""
                "" % (switch_id[key],switch_id[key])) 
                STORE_MONITERED_ALIVE_REG_ACTION_X2.append("""
                    if(signatureNow==%d){
                        STORE_MONITERED_ALIVE_REG_ACTION(%d);"""
                         "" % (switch_id[key],switch_id[key]))
                neighborNum=0
                for neighbor in list(neighbor_graph_list[int(value)].keys()):
                        ...
                        STORE_MONITERED_ALIVE_REG_ACTION_X2.append("""
                    STORE_MONITERED_ALIVE_REG_ACTION_X(%d,%d,%d)"""
                        "" % (neighborNum,neighbor,switch_id[key]))
                        neighborNum+=1 
                STORE_MONITERED_ALIVE_REG_ACTION_X2.append("""
                    }"""
                ) 
                ...
                generated_text1.append("""
                    else if(reg_table_key_monitered==%d){
                        STORE_MONITERED_ALIVE_REG_ACTION_N(%d);
                    }"""
                "" % (switch_id[key],switch_id[key])) 
                generated_text_control.append("""
            ...
            if neighbor_num == '0':
                generated_text3.append("""
                if(next_switch_reg_key==%d){
                    state_all=  get_neighbor_alive_state_string_%d_reg_action.execute(1);"""
                "" % (switch_id[key],switch_id[key])) 
                
                generated_text3.append("""
                    timeout_next_Switch1 =check_neighbor_time_%d_reg_action.execute(%d);"""
                "" % (switch_id[key],switch_id['SWITCH_NUM'])) 
                neighborNum=2
                for neighbor in list(neighbor_graph_list[int(switch_id[key])].keys()):
                        if neighbor==11:continue
                        generated_text3.append("""
                    timeout_next_Switch%d =check_neighbor_time_%d_reg_action.execute(%d);"""
                        "" % (neighborNum,switch_id[key],neighbor))
                        neighborNum+=1 
                generated_text3.append("""
                    }"""
                )        
    ...
    #         timeout_next_Switch2 =check_neighbor_time_11_reg_action.execute(%d);
    #     }"""
    # "" % (switch_id['SWITCH_NUM'])) 
    
    
    
    
    with open(f"pktgen_control{IP_SwitchCode[switch_id['SWITCH_NUM']]}_{switch_id['SWITCH_NUM']}.p4", "w") as f:
        print("""%s"""
          "" % ('_'.join(switchNUM)),
        file=f)
        print("""(
        ...
        bit<1> timeout_next_Switch7=0;
        bit<1> timeout_next_Switch8=0;
        ...
        """
          "" % (''.join(mask),
                ''.join(GEN_REGISTER_TIME),
                ''.join(GEN_NEIGHBOR_STATE_STRING_REGISTER),
                ),
                file=f)
        print("""     
              %s
        ...
        print("""     
              ...
              """
          "" % ('\n'.join(generated_text1),
                '\n'.join(generated_text_control),
                '\n'.join(STORE_MONITERED_ALIVE_REG_ACTION_X2),

                ),
        file=f)
        BITLOCATION=SWITCH_NUM-1
        print(BITLOCATION)
        print("""     
            else{
              ...
              // }
                ...
                //todo
                next_Switch_state= (register_state_t)(state_all[10:10] ++ state_all[9:0]);
        ...
        ig_intr_tm_md.bypass_egress = 1w1;
              ...
              """
          "" % ('\n'.join(generated_text2),
                '\n'.join(generated_text3),),
        file=f)