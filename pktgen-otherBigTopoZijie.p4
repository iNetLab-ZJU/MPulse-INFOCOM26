...
control SwitchIngress(
    ...
    //the table of common neighbor id
    Register<bit<32>, reg_table_key_t>(node_num) common_neighbor_id_reg;
    RegisterAction<bit<32>, reg_table_key_t, bit<32>>(neighbor_id_reg) 
    read_common_neighbor_id_reg_action = {
        void apply(inout bit<32> value, out bit<32> read_value){
            read_value = value;
        }
    };
    ...
    action common_neighbors_action(bit<32> neighbor_list){
        common_neighbor_0=neighbor_list[3:0];
        common_neighbor_1=neighbor_list[7:4];
        common_neighbor_2=neighbor_list[11:8];
        common_neighbor_3=neighbor_list[15:12];
    }
...
//todo:
        // n : monitered switch id, signature:1
    ...
    #define STORE_MONITERED_ALIVE_REG_ACTION(n)\
            ...
            store_neighbor_time_##n##_reg_action.execute(1);
    #endif
 ...
 //todo:
    ...
    table common_neighbors_table {
        key = {
            hdr.heartbeat.signature : exact;
            hdr.heartbeat.Monitored_switch : exact;
        }
        actions = {
            common_neighbors_action;
            @defaultonly drop;
        }
        const default_action = drop();
        size = 1024;
        ...
        // if(ig_intr_md.resubmit_flag==0){
            ...
            if (hdr.timer.isValid()) {
                ...
                if(batch_time==0){
                    ...
                    if(reg_table_key_monitered==5){
                        STORE_MONITERED_ALIVE_REG_ACTION_N(5);
                    }else if(reg_table_key_monitered==6){
                        STORE_MONITERED_ALIVE_REG_ACTION_N(6);
                    }else if(reg_table_key_monitered==7){
                        STORE_MONITERED_ALIVE_REG_ACTION_N(7);
                    }else if(reg_table_key_monitered==8){
                        STORE_MONITERED_ALIVE_REG_ACTION_N(8);
                    ...
                    if(reg_table_key_monitered==11){
                        ...
                        STORE_MONITERED_ALIVE_REG_ACTION_N(11);
                        GET_NEIGHBOR_ALIVE_REG_ACTION(0, 5 ,1);
                        GET_NEIGHBOR_ALIVE_REG_ACTION(1, 6 ,1);
                        GET_NEIGHBOR_ALIVE_REG_ACTION(2, 7 ,1);
                        GET_NEIGHBOR_ALIVE_REG_ACTION(3, 8 ,1);
                    }else{
                        if(reg_table_key_monitered==5){
                            STORE_MONITERED_ALIVE_REG_ACTION_N(5);
                        }else if(reg_table_key_monitered==6){
                        ...
                        GET_NEIGHBOR_ALIVE_REG_ACTION(0, 11, 1);
                    }   
                    ...
                    if(hdr.heartbeat.heartbeat_type==ACK){
                        ...
                        signatureNow=(reg_table_key_t) hdr.heartbeat.signature;
                         if(signatureNow==11){
                                ...
                                STORE_MONITERED_ALIVE_REG_ACTION_X(0 , 5 , 11);
                                STORE_MONITERED_ALIVE_REG_ACTION_X(1 , 6 , 11);
                                STORE_MONITERED_ALIVE_REG_ACTION_X(2 , 7 , 11);
                                STORE_MONITERED_ALIVE_REG_ACTION_X(3 , 8 , 11);
                        } else if(signatureNow==5){
                            STORE_MONITERED_ALIVE_REG_ACTION(5);
                            STORE_MONITERED_ALIVE_REG_ACTION_X(0 , 11 , 5);
                        }
                        else if(signatureNow==6){
                            STORE_MONITERED_ALIVE_REG_ACTION(6);
                            STORE_MONITERED_ALIVE_REG_ACTION_X(0 , 11 ,6);
                        }
                        else if(signatureNow==7){
                            STORE_MONITERED_ALIVE_REG_ACTION(7);
                            STORE_MONITERED_ALIVE_REG_ACTION_X(0 , 11 , 7);
                        }else if(signatureNow==8){
                            STORE_MONITERED_ALIVE_REG_ACTION(8);
                            STORE_MONITERED_ALIVE_REG_ACTION_X(0 , 11 , 8);
                        }
                        // get_neighbor_id_and_state();
                        // reg_table_key_monitered= (reg_table_key_t) hdr.heartbeat.Monitored_switch;
                    }
                ...
                if(next_switch_reg_key==5){
                    state_all=  get_neighbor_alive_state_string_5_reg_action.execute(1);
                    timeout_next_Switch1 =check_neighbor_time_5_reg_action.execute(11);
                    timeout_next_Switch2 =check_neighbor_time_5_reg_action.execute(1);
                }else if(next_switch_reg_key==6){
                    state_all=  get_neighbor_alive_state_string_6_reg_action.execute(1);
                    timeout_next_Switch1 =check_neighbor_time_6_reg_action.execute(11);
                    timeout_next_Switch2 =check_neighbor_time_6_reg_action.execute(1);
                }else if(next_switch_reg_key==7){
                    state_all=  get_neighbor_alive_state_string_7_reg_action.execute(1);
                    timeout_next_Switch1 =check_neighbor_time_7_reg_action.execute(11);
                        timeout_next_Switch2 =check_neighbor_time_7_reg_action.execute(1);
                }else if(next_switch_reg_key==8){
                    state_all=  get_neighbor_alive_state_string_8_reg_action.execute(1);
                    timeout_next_Switch1 =check_neighbor_time_8_reg_action.execute(11);
                        timeout_next_Switch2 =check_neighbor_time_8_reg_action.execute(1);
                    ...
                    timeout_next_Switch1 =check_neighbor_time_11_reg_action.execute(11);
                        timeout_next_Switch2 =check_neighbor_time_11_reg_action.execute(1);
                }
                // }
