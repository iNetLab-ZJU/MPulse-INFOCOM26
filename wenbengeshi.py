...
# 定义文本格式
text_format = """\
parser SwitchIngressParser_{SWITCH_NUM}(
       packet_in packet, 
       out headers hdr, 
...
control SwitchIngressDeparser_{SWITCH_NUM}(
        packet_out pkt,
        inout headers hdr,
...
control SwitchIngress_{SWITCH_NUM}(
        inout headers hdr, 
    ...
    bit<1> ifTimeOut_1=0;
    bit<1> ifTimeOut_2=0;
    bit<32> mask_{neighbor0};
    bit<32> mask_{neighbor1};
    bit<32> mask_{neighbor2};
    bit<32> mask_{neighbor3};
    bit<32> mask_11;
    ...
    {GEN_NEIGHBOR_STATE_STRING_REGISTER}
        ...
        // if(ig_intr_md.resubmit_flag==0){
            ...
            if (hdr.timer.isValid()) {
                ...
                if(batch_time==0){
                    ...
                    if(reg_table_key_monitered=={neighbor0}}){
                        STORE_MONITERED_ALIVE_REG_ACTION_N({neighbor0});
                    }else if(reg_table_key_monitered=={neighbor1}){
                        STORE_MONITERED_ALIVE_REG_ACTION_N({neighbor1});
                    }else if(reg_table_key_monitered=={neighbor2}){
                        STORE_MONITERED_ALIVE_REG_ACTION_N({neighbor2});
                    }else if(reg_table_key_monitered=={neighbor3}){
                        STORE_MONITERED_ALIVE_REG_ACTION_N({neighbor3});
                    }
                    ...
                    if(reg_table_key_monitered==11){
                        ...
                        GET_NEIGHBOR_ALIVE_REG_ACTION(0, {neighbor0} ,{SWITCH_NUM});
                        GET_NEIGHBOR_ALIVE_REG_ACTION(1, {neighbor1} ,{SWITCH_NUM});
                        GET_NEIGHBOR_ALIVE_REG_ACTION(2, {neighbor2} ,{SWITCH_NUM});
                        GET_NEIGHBOR_ALIVE_REG_ACTION(3, {neighbor3} ,{SWITCH_NUM});
                    }else{
                        if(reg_table_key_monitered=={neighbor0}){
                            STORE_MONITERED_ALIVE_REG_ACTION_N({neighbor0});
                        }else if(reg_table_key_monitered=={neighbor1}){
                        ...
                        neighbor_2=0;
                        neighbor_3=0;
                        GET_NEIGHBOR_ALIVE_REG_ACTION(0, 11, {SWITCH_NUM});
                    }   
                    ...
                    if(hdr.heartbeat.heartbeat_type==ACK){
                        ...
                        signatureNow=(reg_table_key_t) hdr.heartbeat.signature;
                         if(signatureNow==11){
                                ...
                                STORE_MONITERED_ALIVE_REG_ACTION_X(0 , {neighbor0} , 11);
                                STORE_MONITERED_ALIVE_REG_ACTION_X(1 , {neighbor1} , 11);
                                STORE_MONITERED_ALIVE_REG_ACTION_X(2 , {neighbor2} , 11);
                                STORE_MONITERED_ALIVE_REG_ACTION_X(3 , {neighbor3} , 11);
                        } else if(signatureNow=={neighbor0}){
                            STORE_MONITERED_ALIVE_REG_ACTION({neighbor0});
                            STORE_MONITERED_ALIVE_REG_ACTION_X(0 , 11 , {neighbor0});
                        }
                        else if(signatureNow=={neighbor1}){
                            STORE_MONITERED_ALIVE_REG_ACTION({neighbor1});
                            STORE_MONITERED_ALIVE_REG_ACTION_X(0 , 11 ,{neighbor1});
                        }
                        else if(signatureNow=={neighbor2}){
                            STORE_MONITERED_ALIVE_REG_ACTION({neighbor2});
                            STORE_MONITERED_ALIVE_REG_ACTION_X(0 , 11 , {neighbor2});
                        }else if(signatureNow=={neighbor3}){
                            STORE_MONITERED_ALIVE_REG_ACTION({neighbor3});
                            STORE_MONITERED_ALIVE_REG_ACTION_X(0 , 11 , {neighbor3});
                        }
                        // get_neighbor_id_and_state();
                        // reg_table_key_monitered= (reg_table_key_t) hdr.heartbeat.Monitored_switch;
                    }
                        ...
                        reg_table_key_monitered= (reg_table_key_t) hdr.heartbeat.Monitored_switch;
                         ...
                         if(reg_table_key_monitered=={neighbor0}){
                            ...
                            STORE_MONITERED_ALIVE_REG_ACTION({neighbor3});
                        }else if(reg_table_key_monitered==11){
                            STORE_MONITERED_ALIVE_REG_ACTION(11);
                        }
                        drop();
                    }
                ...
                if(next_switch_reg_key=={neighbor0}){
                    state_all=  get_neighbor_alive_state_string_{neighbor0}_reg_action.execute(1);
                    timeout_next_Switch1 =check_neighbor_time_{neighbor0}_reg_action.execute(11);
                    timeout_next_Switch2 =check_neighbor_time_{neighbor0}_reg_action.execute({SWITCH_NUM});
                }else if(next_switch_reg_key=={neighbor1}){
                    state_all=  get_neighbor_alive_state_string_{neighbor1}_reg_action.execute(1);
                    timeout_next_Switch1 =check_neighbor_time_{neighbor1}_reg_action.execute(11);
                    timeout_next_Switch2 =check_neighbor_time_{neighbor1}_reg_action.execute({SWITCH_NUM});
                }else if(next_switch_reg_key=={neighbor2}){
                    state_all=  get_neighbor_alive_state_string_{neighbor2}_reg_action.execute(1);
                    timeout_next_Switch1 =check_neighbor_time_{neighbor2}_reg_action.execute(11);
                        timeout_next_Switch2 =check_neighbor_time_{neighbor2}_reg_action.execute({SWITCH_NUM});
                }else if(next_switch_reg_key=={neighbor3}){
                    state_all=  get_neighbor_alive_state_string_{neighbor3}_reg_action.execute(1);
                    timeout_next_Switch1 =check_neighbor_time_{neighbor3}_reg_action.execute(11);
                        timeout_next_Switch2 =check_neighbor_time_{neighbor3}_reg_action.execute({SWITCH_NUM});
                    ...
                    timeout_next_Switch1 =check_neighbor_time_11_reg_action.execute(11);
                        timeout_next_Switch2 =check_neighbor_time_11_reg_action.execute({SWITCH_NUM});
                }
                // }
...
parser EgressParser_{SWITCH_NUM}(packet_in      pkt,
    /* User */
...
control EgressDeparser_{SWITCH_NUM}(packet_out pkt,
    /* User */
    inout my_egress_headers_t                       hdr,
...
# 定义数组存储不同情况下的值
def generate_switch_id_recognition():
    ...
    # SWITCH_NUM = 0
    moniterSwitch = 11
    neighbor_keys = ["neighbor0", "neighbor1", "neighbor2", "neighbor3"]
    neighbor_11=["neighbor0", "neighbor1", "neighbor2", "neighbor3", "neighbor4", "neighbor5", "neighbor6", "neighbor7", "neighbor8", "neighbor9", "neighbor10"]
    # 初始化结果数组
    result = []
    # 获取指定SwitchNUM的邻居信息
    for SWITCH_NUM in neighbor_graph_list:
        neighbors = neighbor_graph_list[SWITCH_NUM]
        # 遍历邻居信息的每个key
        neighbor_info = {
        ...
        if SWITCH_NUM!=11:
            for i, key in enumerate(neighbors.keys()):
                if i > 0 and i >= len(neighbors):
                    break
                # 构建每个邻居的信息字典
                if key!=11:
                    neighbor_info[neighbor_keys[i]]=key
                i=i+1
        else :
            for i, key in enumerate(neighbors.keys()):
                print(i)
                ...
                # 构建每个邻居的信息字典
                if key!=11:
                    neighbor_info[neighbor_11[i]]=key
                i=i+1
        result.append(neighbor_info)
    print(result)
    return result

switch_id_recognition = generate_switch_id_recognition()

for switch_id in switch_id_recognition:
        # 生成文本文件
    with open("output.txt", "w") as file:
        file.write(text_format.format(**switch_id))

    # 生成匹配项文本并嵌入到文本格式中
    GEN_REGISTER_TIME = "\n".join([f"GEN_REGISTER_TIME({{neighbor{key[-1]}}})" for key in switch_id.keys() if key.startswith('neighbor')])
    GEN_NEIGHBOR_STATE_STRING_REGISTER = "\n".join([f"GEN_NEIGHBOR_STATE_STRING_REGISTER({{neighbor{key[-1]}}})" for key in switch_id.keys() if key.startswith('neighbor')])
