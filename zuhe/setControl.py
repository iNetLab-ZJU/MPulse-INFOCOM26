...
def setFlow(bfrt_info, interface, ip,pipeId):
    ...
    ###
    pktgen_self_timeFlow = bfrt_info.table_get("pipe_a.SwitchIngress_c.timer_periodic")
    # port_down_table = bfrt_info.table_get("pipe_a.SwitchIngress_c.port_down_table")
    pktgen_heart_back = bfrt_info.table_get("pipe_a.SwitchIngress_c.heart_back")
    # receive_heartbeat = bfrt_info.table_get("pipe_a.SwitchIngress_c.receive_heartbeat")
    # neighbor_alive_reg = bfrt_info.table_get("pipe_a.SwitchIngress_c.neighbor_alive_reg")
    # receive_mulicast = bfrt_info.table_get("pipe_a.SwitchIngress_c.receive_mulicast")
    # change_port_table = bfrt_info.table_get("pipe_a.SwitchIngress_c.change_port_table")
    ###
...
for x in ipList:
    ...
    learn_filter = bfrt_info[x].learn_get("pipe_a.SwitchIngressDeparser_c.digest")
    ...
    learn_filter_b = bfrt_info[x].learn_get("pipe_b.SwitchIngressDeparser.digest")
    learn_filter_b.info.data_field_annotation_add("signature", "")
    learn_filter_b.info.data_field_annotation_add("port_0", "")
    learn_filter_b.info.data_field_annotation_add("error_switch_0", "")
    learn_filter_b.info.data_field_annotation_add("port_1", "")
    learn_filter_b.info.data_field_annotation_add("error_switch_1", "")
    check_digest_thread = threading.Thread(target=check_digest,args=(interface["192.168.123.103"],learn_filter,0,))
    check_digest_thread.start()
    check_digest_thread_c = threading.Thread(target=check_digest,args=(interface["192.168.123.103"],learn_filter_b,1,))
    check_digest_thread_c.start()
