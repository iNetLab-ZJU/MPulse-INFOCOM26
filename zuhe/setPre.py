...
def setNeighborRegister(bfrt_info, interface, ip):
    ...
    # neighbor_id_reg = bfrt_info.table_get("pipe.SwitchIngress_c.neighbor_id_reg")
    # for i in neighbor_graph:
    #     for m in range(2):
    #         neighborNode_list = neighbor_graph[i][2*m:2*(m+1)]#0:2,2:4
    #         sum_N=0
    ...
    #         print(sum_N)
    #         id=2*i+m
    #         AddRegisterData(bfrt_info,target,neighbor_id_reg,id,sum_N)
...
def setFlow(bfrt_info, interface, ip):
    ...
    ###
    pktgen_self_timeFlow = bfrt_info.table_get("pipe.SwitchIngress_c.timer_periodic")
    # port_down_table = bfrt_info.table_get("pipe.SwitchIngress_c.port_down_table")
    pktgen_heart_back = bfrt_info.table_get("pipe.SwitchIngress_c.heart_back")
    # receive_heartbeat = bfrt_info.table_get("pipe.SwitchIngress_c.receive_heartbeat")
    neighbor_alive_reg = bfrt_info.table_get("pipe.SwitchIngress_c.neighbor_alive_reg")
    # receive_mulicast = bfrt_info.table_get("pipe.SwitchIngress_c.receive_mulicast")
    change_port_table = bfrt_info.table_get("pipe.SwitchIngress_c.change_port_table")
    ###
    ...
    for pipeId in range(num_pipes):
        ...
        # pktgen_heart_back_data = pktgen_heart_back.make_data([], 'SwitchIngress_c.heart_back_action')
        ...
        for batch in range(b_count):
            for pkt_num in range(n_count):
                for i in range(2):
                    ...
                    pktgen_self_timeFlow_data = pktgen_self_timeFlow.make_data(
                        ...
                        gc.DataTuple('trans_neighbor_alive_list',sum_N)],'SwitchIngress_c.match_consensus')
                    pktgen_self_timeFlow.entry_add(target, [pktgen_self_timeFlow_key], [pktgen_self_timeFlow_data])
                    pkt_now+=1
            # pkt_num=n_count*2
            # if signature in trans_node:
            #     for transTarget in trans_node[signature]:  
            #         if transTarget =="port":
            #             continue
            #         pktgen_self_timeFlow_key = pktgen_self_timeFlow.make_key(
            #             [gc.KeyTuple('hdr.timer.app_id', app_id), gc.KeyTuple('ig_intr_md.ingress_port', pipe_port),
            ...
            #         pkt_now+=1
            #         sum_N=0
            #         transPort_list=trans_node[signature][transTarget]
            #         for j in range(len(transPort_list)):
            #             sum_N=sum_N+transPort_list[j]*2**(j*4)
            #         print("*"*24)
            #         print(sum_N)
            #         if transTarget==111 or transTarget==112 or transTarget==113:
            #             transTarget=11
            #         if transTarget==121 or transTarget==122:
            #             transTarget=12
            #         pktgen_self_timeFlow_data = pktgen_self_timeFlow.make_data(
            #             [gc.DataTuple('port', trans_node[signature]["port"]),
            #             gc.DataTuple('Monitored_switch', transTarget),
            #             gc.DataTuple('signature', signature), 
            #             gc.DataTuple('trans_neighbor_alive_list',sum_N)],'SwitchIngress_c.match_consensus')
            #         pktgen_self_timeFlow.entry_add(target, [pktgen_self_timeFlow_key], [pktgen_self_timeFlow_data])
        p_count=pkt_now
        ...
        #             [gc.KeyTuple('hdr.timer.app_id', port_down_app_id), gc.KeyTuple('ig_intr_md.ingress_port', ),
        #             gc.KeyTuple('hdr.timer.pipe_id', pipeId), gc.KeyTuple('hdr.timer.batch_id', batch),
        #             gc.KeyTuple('hdr.timer.packet_id', pkt_num)])
        # port_down_table_data = port_down_table.make_data(
        #     [gc.DataTuple('port', neighborNode_port[neighborNode_list[pkt_num]]),
        #     gc.DataTuple('Monitored_switch', neighborNode_list[pkt_num]),
        #     gc.DataTuple('signature', signature)], 'SwitchIngress_c.match_down')
        # port_down_table.entry_add(target, [port_down_table_key], [port_down_table_data])
    
        


def check_digest(interface,learn_filter):
    while True:
        try:
...
learn_filter = bfrt_info["192.168.123.103"].learn_get("digest")
...
check_digest_thread = threading.Thread(target=check_digest,args=(interface["192.168.123.103"],learn_filter,))
