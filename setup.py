...
def setFlow(bfrt_info, interface, ip):
    ...
    pktgen_app_key = pktgen_app.make_key([gc.KeyTuple('app_id', link_down_app_id)])
...
def breakPort(bfrt_info, devPort):
    target = gc.Target(device_id=0, pipe_id=0xffff)  
    port_table = bfrt_info.table_get("port.port")
    port_table.entry_mod(
                target,
                [port_table.make_key([
                    gc.KeyTuple('$DEV_PORT', devPort)])],
                [port_table.make_data([
                    gc.DataTuple('$PORT_ENABLE', 0)])]
            )

def recoverPort(bfrt_info, devPort):
    target = gc.Target(device_id=0, pipe_id=0xffff)  
    port_table = bfrt_info.table_get("port.port")
    port_table.entry_mod(
                target,
...
switch = input("请输入交换机号: ")
num2 = input("请输入另一方连接交换机: ")

signature, neighborNode_list, neighborNode_port, ip = switch_id_recognition(switch)
x_port = neighborNode_port.get(num2)

breakPort(bfrt_info[ip],x_port)
